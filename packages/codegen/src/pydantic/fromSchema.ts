...
export function jsonSchemaToPydantic(schema: SchemaNode, opts?: { className?: string }): string {
...
function generateModel(schema: SchemaNode, className: string, ctx: GeneratorContext): void {
  ...
  for (const [propName, propSchema] of Object.entries(properties)) {
    ...
    const { typeStr, fieldArgs } = resolvePropertyType(propSchema, propName, className, ctx);
    ...
    if (!isRequired) {
      ...
      lines.push(`    ${pythonName}: ${typeStr} = Field(..., ${allFieldArgs.join(', ')})`);
    } else {
...
function resolvePropertyType(
  ...
  if (typeof schema['description'] === 'string') {
    fieldArgs.push(`description="${escapeString(schema['description'])}"`);
  }
  ...
  if (typeof schema['pattern'] === 'string') {
    fieldArgs.push(`pattern="${escapeString(schema['pattern'])}"`);
  }
