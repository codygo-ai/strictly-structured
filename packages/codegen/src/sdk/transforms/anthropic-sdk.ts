...
import { addChange, appendToDescription, deepClone, isObjectNode, walkAllObjects } from '../shared';
 ...
 */
export function simulateAnthropicSdk(schema: Record<string, unknown>): SdkTransformResult {
  ...
  walkAllObjects(transformed, '', (node, path) => {
    ...
    if (Array.isArray(node['oneOf'])) {
      ...
      delete node['oneOf'];
      addChange(changes, `${path}/oneOf`, 'modified', `Rewrote oneOf to anyOf`, 'oneOf', 'anyOf');
    }
...
function checkAllFieldsRequired(schema: Record<string, unknown>, gaps: SdkGap[]): void {
  let hasGap = false;
