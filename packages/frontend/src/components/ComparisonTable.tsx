...
function cellStyle(ok: boolean | 'warn' | 'partial'): {
  ...
  if (ok === false)
    return { color: 'var(--ds-cell-fail-text)', background: 'var(--ds-cell-fail-bg)' };
      ...
      <table className="w-full border-collapse text-[13px]">
        <thead>
          <tr className="border-b-2 border-border">
            <th className="text-left py-2.5 px-3 font-semibold text-secondary text-xs">Feature</th>
            {columns.map((col) => (
              <th key={col.id} className="text-center py-2.5 px-3 font-bold text-primary text-xs">
        ...
        <tbody>
          {rows.map((row, i) => (
            ...
            >
              <td className="py-2 px-3 text-secondary font-medium text-[12.5px]">{row.feature}</td>
              {columns.map((col) => {
