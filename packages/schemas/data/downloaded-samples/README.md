...
This directory holds **70 JSON schema samples per model group** fetched from the [JSONSchemaBench](https://huggingface.co/datasets/epfl-dlab/JSONSchemaBench) dataset (EPFL Data Science Lab). They are intended as a **basis for test cases** for group meta-schema validation.
...
1. Validate each sample against the group meta-schema (e.g. via `groupMetaSchema.validation.test.ts` or CLI).
2. Set `_meta.expected` to `"valid"` or `"invalid"` and optionally `expectErrorPattern` for invalid cases.
3. Optionally copy or move selected files into `group-meta-schema-tests/{groupId}/` and wire them into `getSamples()` / `buildSamples()` as needed.
