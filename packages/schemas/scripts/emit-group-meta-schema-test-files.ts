/**
 * Emits one JSON file per test case under data/group-meta-schema-tests/{groupId}/{slug}.json.
...
function main(): void {
  ...
  for (const s of samples) {
    const dir = path.join(OUT_DIR, s.groupId);
    if (!byGroup.has(s.groupId)) {
      fs.mkdirSync(dir, { recursive: true });
      byGroup.set(s.groupId, dir);
    }
  ...
  for (const g of groups) {
    const count = samples.filter((s) => s.groupId === g).length;
