...
export function formatRuleSetAsText(ruleSet: SchemaRuleSet): string {
  ...
  const rootTypes = Array.isArray(ruleSet.rootType)
    ? ruleSet.rootType.join(' | ')
    : ruleSet.rootType;
  sections.push(
    [
      '## Root Constraints',
  ...
  limitLines.push(
    ruleSet.sizeLimits.maxProperties !== null
      ? `- Max properties: ${ruleSet.sizeLimits.maxProperties}`
      : '- Max properties: no limit',
  );
  limitLines.push(
    ruleSet.sizeLimits.maxNestingDepth !== null
      ? `- Max nesting depth: ${ruleSet.sizeLimits.maxNestingDepth}`
      : '- Max nesting depth: no limit',
  );
  if (
    ruleSet.sizeLimits.maxStringLengthNamesEnums !== undefined &&
    ruleSet.sizeLimits.maxStringLengthNamesEnums !== null
  ) {
    limitLines.push(
      `- Max string length (names + enums): ${ruleSet.sizeLimits.maxStringLengthNamesEnums}`,
    );
  }
...
export function formatMarkersAsText(markers: SchemaMarker[]): string {
  ...
  return markers
    .map(
      (m, i) =>
        `${i + 1}. [${m.severity}] Line ${m.startLineNumber}:${m.startColumn} — ${m.message}`,
    )
    .join('\n');
}
