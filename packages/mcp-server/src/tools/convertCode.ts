...
export function registerConvertCodeTool(server: McpServer): void {
  server.tool(
    ...
    'Convert between Zod code, Pydantic code, and JSON Schema. Supports bidirectional conversion.',
    {
      ...
      to: z.enum(['zod', 'pydantic', 'json-schema']).describe('Target format to convert to'),
    },
    async ({ code, from, to }) => {
              ...
              text: JSON.stringify(
                {
                  result: code,
                  warnings: [
                    { message: 'Source and target formats are the same', severity: 'info' },
                  ],
                },
                undefined,
                2,
          ...
          content: [
            {
              ...
              text: JSON.stringify(
                {
                  error:
                    sourceFormat === 'pydantic' || to === 'pydantic'
