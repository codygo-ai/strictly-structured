  ...
  server.tool(
    ...
    async ({ code, format, sdk, providers }) => {
      try {
        ...
        // Step 1: Convert Zod → raw JSON Schema
        const conversion = zodToJsonSchema(code);
        const rawSchema = conversion.schema;

        // Step 2: For each provider, optionally simulate SDK transform, then validate
        const groups = getGroupsByProviders(providers as ProviderId[] | undefined);
        const defaultSdkForProvider: Record<string, SdkId> = {
          openai: "openai-sdk",
          anthropic: "anthropic-sdk",
          gemini: "gemini-sdk",
        };

        const providerResults = groups.map((group) => {
          const targetSdk = sdk ?? defaultSdkForProvider[group.providerId] ?? "openai-sdk";

          // Simulate SDK transform
          ...
          // Validate transformed schema against provider rules
          const markers = validateSchemaForGroup(transformedSchemaStr, group);
