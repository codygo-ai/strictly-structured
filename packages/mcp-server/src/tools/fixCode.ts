...
import { zodToJsonSchema, jsonSchemaToZod, detectFormat } from '@ssv/codegen';
...
export function registerFixCodeTool(server: McpServer): void {
  server.tool(
    ...
    'Fix Zod or Pydantic code to make it compatible with a specific LLM provider. Converts to JSON Schema, applies fixes, then converts back to original language.',
    {
      ...
      provider: z.enum(['openai', 'anthropic', 'gemini']).describe('Target provider to fix for'),
    },
            ...
            content: [
              {
                ...
                text: JSON.stringify(
                  {
                    error:
                      'Input looks like JSON Schema. Use fix_schema instead, which accepts raw JSON Schema directly.',
                  },
                  undefined,
                  2,
                ),
            ...
            content: [
              {
                ...
                text: JSON.stringify(
                  {
                    error:
                      'Pydantic code fixing requires Python runtime. Paste JSON Schema and use fix_schema instead.',
                  },
                  undefined,
                  2,
                ),
