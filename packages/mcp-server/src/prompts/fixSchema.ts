  ...
  server.registerPrompt(
    'fix-schema',
    {
      ...
      argsSchema: {
        ...
        provider: z.enum(['openai', 'anthropic', 'gemini']).describe('Target LLM provider'),
      },
    },
    async ({ schema, provider }) => {
      ...
      if (!ruleSet) {
        return {
          messages: [
            {
              role: 'user' as const,
              content: { type: 'text' as const, text: `Unknown provider: ${provider}` },
            },
          ],
        };
      }
      ...
      try {
        ...
        return {
          messages: [
            {
              role: 'user' as const,
              content: { type: 'text' as const, text: 'Schema is not valid JSON.' },
            },
          ],
        };
      }
          ...
          messages: [
            {
              ...
              content: {
                type: 'text' as const,
                text: 'Schema must be a JSON object, not a primitive or array.',
              },
            },
          ],
        };
      ...
      const errorsText =
        markers.length > 0 ? formatMarkersAsText(markers) : 'Schema is not valid JSON.';

      const appliedFixesList =
        fixResult.appliedFixes.length > 0
          ? fixResult.appliedFixes.map((f, i) => `${i + 1}. ${f.description}`).join('\n')
          : 'None';

      const remainingIssuesList =
        postFixMarkers.length > 0 ? formatMarkersAsText(postFixMarkers) : 'None';
