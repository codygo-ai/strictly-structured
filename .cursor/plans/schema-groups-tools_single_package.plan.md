...
overview: Single package schemas (data + CLI) owns canonical data and generates artifacts; consumers pass the output path via CLI argument; frontend/backend use only generated .generated.* files. No schema-utils.
todos: []
isProject: false
---

# Single schemas: data + tools, generated artifacts only

## Goals

1. **One package: schemas** – Holds both the canonical **data** (`structured_output_groups.json`) and **tools** (CLI). Used as a CLI; consumers do not import from it at runtime.
...
5. **No runtime sharing for now** – Frontend and backend have everything they need inside their package (generated files + their code). No dependency on schemas at runtime.

---

## 1. Package: schemas

**Location**: [packages/schemas](packages/schemas)  
**Name**: `@ssv/schemas`
...
- Example: `pnpm --filter @ssv/schemas generate --out-dir packages/frontend/src/data`
- Or: `pnpm exec schemas generate --to ./src/generated`
...
Frontend and backend **do not** depend on `@ssv/schemas` at runtime. They run the CLI (e.g. in a script or CI) with their chosen path and then use only the generated files in their package.
...
- No dependency on schema-utils or schemas at runtime.
- A script or docs instruct: run schemas with e.g. `--out-dir packages/frontend/src` (or a path that yields `src/data/` and `src/types/` as desired).
...
- Remove or deprecate [packages/schema-utils](packages/schema-utils). [devops/compatibility-runner](devops/compatibility-runner) migrates to schemas for types (or gets a generated types file by running the CLI with its own `--out-dir`).

---

## 4. Migration steps (order)

1. **Create packages/schemas** with data, CLI (with **required** `--out-dir`/`--to`), schema samples, and provider meta-schema generator. CLI must require the output path and document the exact filenames it writes under that path.
2. **Frontend**: Add a script that runs the CLI with e.g. `--out-dir packages/frontend/src` (or the path that matches frontend layout). Switch imports to generated files; remove old groups JSON. No runtime dependency on schemas.
3. **Backend**: If needed, run CLI with backend’s path; use only generated files.
4. **Compatibility-runner**: Switch from schema-utils to schemas (or run CLI with compatibility-runner’s path for generated types).
5. **Remove schema-utils** and root schema-samples; update pnpm-workspace.
6. **Docs**: README in schemas states that consumers must pass the output path via argument; list generated filenames and formats.

---

## 5. Summary

- **Single package**: schemas = data + CLI + schema samples + provider meta-schema generation.
